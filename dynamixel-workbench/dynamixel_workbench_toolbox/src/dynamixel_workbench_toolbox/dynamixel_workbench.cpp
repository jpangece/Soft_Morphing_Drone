...
bool DynamixelWorkbench::setMultiTurnControlMode(uint8_t id, const char **log)
{
  ...
  {
    if (log != NULL)
      *log = "[DynamixelWorkbench] Failed to set Multi-Turn Control Mode!";
    return false;
  }

  if (log != NULL)
    *log = "[DynamixelWorkbench] Succeeded to set Multi-Turn Control Mode!";
  return result;
}
...
bool DynamixelWorkbench::getPresentVelocityData(uint8_t id, int32_t *data, const char **log)
{
  ...
  {
    if (log != NULL)
      *log = "[DynamixelWorkbench] Failed to get present speed data!";
    return result;
  }

  *data = get_data;
  if (log != NULL)
    *log = "[DynamixelWorkbench] Succeeded to get present speed data!";
  return result;
}
  ...
  if (getProtocolVersion() == 1.0f)
  {
        ...
        strncmp(getModelName(id), "MX", strlen("MX")) == 0)
    {
      if (velocity == 0.0f)
        ...
        value = (velocity / (model_info->rpm * RPM2RADPERSEC)) + 1023;
...
float DynamixelWorkbench::convertValue2Velocity(uint8_t id, int32_t value)
{
  ...
  if (getProtocolVersion() == 1.0f)
  {
        ...
        strncmp(getModelName(id), "MX", strlen("MX")) == 0)
    {
      ...
      else if (value > 1023 && value < 2048)
        velocity = (value - 1023) * model_info->rpm * RPM2RADPERSEC * (-1.0f);
...
float DynamixelWorkbench::convertValue2Current(uint8_t id, int16_t value)
{
  float current = 0;
  // float CURRENT_UNIT = 2.69f; // Unit : mA, Ref : http://emanual.robotis.com/docs/en/dxl/x/xm430-w350/#goal-current102
  float CURRENT_UNIT = 1.00f; // Unit : mA, Ref : https://emanual.robotis.com/docs/en/dxl/x/xl330-m288/#goal-current102
  ...
  else if (getProtocolVersion() == 2.0f)
  {
    ...
    {
      CURRENT_UNIT = 1.0f;
      std::cout << "using 1.0 ma unit" << std::endl;
      current = (int16_t)value * CURRENT_UNIT;
      return current;
    }
    else
...
float DynamixelWorkbench::convertValue2Load(int16_t value)
{
  ...
  if (value == 1023 || value == 0)
    ...
    load = value * LOAD_UNIT;
  else if (value > 1023 && value < 2048)
    load = (value - 1023) * LOAD_UNIT * (-1.0f);
